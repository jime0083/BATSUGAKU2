 ...
 * 通算日数の達成マイルストーンをチェック
 */
function checkTotalDaysAchievement(totalDays, postedMilestones = []) {
 ...
 * 連続日数の達成マイルストーンをチェック
 */
function checkStreakAchievement(streakDays, postedMilestones = []) {
    const postedSet = new Set(postedMilestones);
 ...
 * GitHub pushイベントを処理
 */
async function handleGitHubPush(senderUsername, xClientId, xClientSecret) {
    var _a, _b;
    ...
    if (statsUpdated && user.xLinked && user.xAccessToken) {
        ...
        let updatedUser = Object.assign(Object.assign({}, updatedUserDoc.data()), { uid: updatedUserDoc.id });
        // トークンの有効期限チェック＆リフレッシュ
        if ((0, twitter_1.isTokenExpired)(updatedUser) && updatedUser.xRefreshToken && xClientId && xClientSecret) {
            console.log('Refreshing X token for user:', user.uid);
            const refreshed = await (0, twitter_1.refreshXToken)(updatedUser.xRefreshToken, xClientId, xClientSecret);
            if (refreshed) {
                await db.collection('users').doc(user.uid).update({
                ...
                updatedUser = Object.assign(Object.assign({}, updatedUser), { xAccessToken: refreshed.accessToken, xRefreshToken: refreshed.refreshToken, xTokenExpiresAt: refreshed.expiresAt });
            }
        }
        // 通算日数達成ツイート
        const totalDaysMilestone = checkTotalDaysAchievement(updatedUser.stats.totalStudyDays, updatedUser.postedTotalDaysMilestones || []);
        if (totalDaysMilestone && updatedUser.xAccessToken) {
            console.log('Posting total days achievement tweet:', totalDaysMilestone);
            const tweetText = (0, twitter_1.generateTotalDaysAchievementText)(updatedUser, totalDaysMilestone);
            const tweetResult = await (0, twitter_1.postTweet)(updatedUser.xAccessToken, tweetText);
        ...
        // 連続日数達成ツイート
        const streakMilestone = checkStreakAchievement(updatedUser.stats.currentStreak, updatedUser.postedStreakMilestones || []);
        if (streakMilestone && updatedUser.xAccessToken) {
            console.log('Posting streak achievement tweet:', streakMilestone);
            const tweetText = (0, twitter_1.generateStreakAchievementText)(updatedUser, streakMilestone);
            const tweetResult = await (0, twitter_1.postTweet)(updatedUser.xAccessToken, tweetText);
            if (tweetResult.success) {
